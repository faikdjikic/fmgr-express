...
module.exports = async function searchDir(req, next, config, rootDir) {
    ...
    const getFiles = await searchGlob(path.join(rootDir, `**/${pattern}`));
    ...
    for (let i = 0; i < getFiles.Files.length; i++) {
        ...
        const currentFile = getFiles.Files[i].indexOf("/") > -1 ? getFiles.Files[i].split("/").reverse()[0] : getFiles.Files[i];
        let currentFilePath = getFiles.Files[i].indexOf("/") > -1 ? (getFiles.Files[i].split("/").slice(0, -1).join("/") + "/").replace(rootDirUrl, "") : getFiles.Files[i].replace(rootDirUrl, "");
        ...
        listedFiles.push({
            ...
            url: [config.fmRootUrl, currentFilePath, encodeURIComponent(currentFile)].filter(item => item != "").join("/"),
            props: fProps
        })
    }
