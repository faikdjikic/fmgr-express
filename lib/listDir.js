...
module.exports = async function listDir(req, next, config, rootDir) {
    ...
    for (let i = 0; i < getFiles.Files.length; i++) {
        ...
        let fileType = config.fileTypes.filter(ftype => {
            ...
            return ftype.extensions.includes(extArr[extArr.length - 1].toLowerCase())
        });
        ...
        listedFiles.push({
            ...
            url: [config.fmRootUrl, ...(activePath.filter(item => item != "")), encodeURIComponent(getFiles.Files[i])].join("/"),
            props: fProps
        })
    }
