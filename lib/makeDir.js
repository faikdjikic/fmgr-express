...
module.exports = async function makeDirectory(req, config, rootDir) {
    ...
    let newPath = `${rootDir}${reqUrl}${reqUrl.charAt(reqUrl.length - 1) == "/" ? "" : "/"}${dirName}`;
    const ifExists = await fsFn.fileExists(newPath);
    if (ifExists) {
        const dirExists = { ErrorCode: -17, ErrorDesc: `Directory "${dirName}" already exists in ${reqUrl == '/' ? "assets root directory" : reqUrl}`, Success: false };
        return dirExists;
    }
    const mkDir = await fsFn.dirCreate(newPath,rootDir);
